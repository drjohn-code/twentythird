...
- *No automated retry worker behind `initial_readings_status = 'pending'`.* Any failure inside `/api/internal/initial-readings` (transient or permanent) flips both `intake_status` and `initial_readings_status` to `'failed'`, which renders `IntakeFailedState` and activates the user's own `POST /api/intake/retry` CTA. Before 2026-07-23 a transient failure (e.g. a JSON-parse failure from `initial_readings` surviving `callAI`'s one built-in retry) instead reset the row back to `'pending'` and returned silently — nothing ever re-scanned it, so the account was stuck forever, indistinguishable from "still generating," and the `room_ready` email deferred for the full 6h before quietly failing. Building a real retry worker (vs. relying on the user's manual retry CTA) is still open — see the fix in `app/api/internal/initial-readings/route.ts`.
